...
import time
import os
from script.services.traitement import traitement
    ...
    try:
     while True:
         ...
         if current_time - start_time >= capture_interval:
             ...
             image_filename = os.path.join(live_folder, f"capture_{int(current_time)}.jpg")
             cv2.imwrite(image_filename, frame)
             print(f"Image capturée: {image_filename}")
             traitement.lancementTraitementDesImagesLive()
